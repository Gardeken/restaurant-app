...
formulario.addEventListener('submit',async (e)=>{
    ...
    const listadoInputs = [inputCat, inputNombre, inputPrecio].some(i => i !== '')
    if(!listadoInputs){
        return alert('No puede dejar los campos vacios')
    }
    await agregarBD({nombre:inputNombre, precio: inputPrecio, categoria: inputCat, id: `${cantidadProductos.length+=1}`})
