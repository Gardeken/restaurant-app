...
formulario.addEventListener('submit', async(e) => {
    e.preventDefault()
    const listadoInputs = [inputCat.value, inputNombre.value, inputPrecio.value].some(i => i === '')
    if(listadoInputs){
        alert('Todos los campos son obligatorios')
        return
    }

    const nombre = inputNombre.value, categoria = inputCat.value, precio = inputPrecio.value,
    confirmar = confirm('Esta seguro de los cambio?')
    if(confirmar){
        window.location.href = '/index.html'
        await editarBD({nombre:nombre, id:id, categoria:categoria, precio:precio})
    }
})
